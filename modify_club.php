...
if(isset($_POST["submit_club"]))
{
    if(!empty($_POST["nom_club"]) && !empty($_POST["date_club"]) && !empty($_POST["description_club"]) && !empty($_FILES["image"]["name"]))
    {
        ...
        // if($Nom_exist->rowCount()==0)
        // {
            $req="update club set Nom=?,Date=?,Description=? where Id=? ";
            $req=$db->prepare($req);
            $req->execute([$nom_club,$date_club,$description_club,$id]);
			header("location:recherch.php?r=".$nom_club);
        // }else{
        //     $erreur="The club already exists!";
        // }
...
<div id="club" class="ajout_club">
    <div class="ajout_title"><h3>Modify Club </h3></div>
	<?php
            ...
            <div class="col-md-4 p-5 shadow-sm border ">
                <form class="form_login" method="POST" action="modify_club.php?id=<?php if(isset($id)) echo $id; ?>#club" enctype="multipart/form-data">
                    ...
                    <div class="mb-3">
                        ...
                        <img src="pages/affiche_img.php?id=<?php echo $infoClubModify["Id"]; ?>" width="100%" height="180" alt="sport" style="border:1px solid rgb(0,0,0,0.13);">
                    </div>
                    <div class="mb-3">
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control" name="description_club" id="exampleFormControlTextarea1" rows="3" placeholder="Club Discription..."><?php if(isset($infoClubModify["Description"])) echo $infoClubModify["Description"]; ?></textarea>
                    </div>
                    <div class="d-grid">
                        <button class="btn btn-primary" name="submit_club" type="submit">Modify club</button>
                    </div>
                </form>
            </div>
			<?php
